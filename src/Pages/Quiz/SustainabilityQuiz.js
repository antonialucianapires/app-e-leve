...
function Quiz({navigation}) {
    const [data, setData] = useState({ quiz1: [] });
    useEffect(() => {
    ...
    [answerD,setAnswerD] = useState(Boolean)
     ...
     return (
        <View style={styles.container}>
            <Text style={styles.question}>{data.quiz1.map(item => (item.question))}</Text>            
            <CheckBox
                title={data.quiz1.map(item => (item.options[0]))}
            ...
            />
            <CheckBox
                title={data.quiz1.map(item => (item.options[1]))}
                containerStyle={styles.checkBox}
                textStyle={styles.textCheckBox}
                checkedColor='#FFFFFF'
                uncheckedColor='#FFFFFF'
            ...
            />
            <CheckBox
                title={data.quiz1.map(item => (item.options[2]))}
                containerStyle={styles.checkBox}
                textStyle={styles.textCheckBox}
                checkedColor='#FFFFFF'
                uncheckedColor='#FFFFFF'
            ...
            />
            <CheckBox
                title={data.quiz1.map(item => (item.options[3]))}
                containerStyle={styles.checkBox}
                textStyle={styles.textCheckBox}
                checkedColor='#FFFFFF'
                uncheckedColor='#FFFFFF'
